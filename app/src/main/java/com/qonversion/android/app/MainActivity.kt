...
class MainActivity : AppCompatActivity() {
    ...
    /**
     * Here is an additional example of how you can retrieve push token for test purposes.
     * The token used for sending messages to the application. The token is the same as the one generated by FirebaseMessagingService.onNewToken().
     */
    private fun setPushToken() {
        FirebaseMessaging.getInstance().token.addOnCompleteListener(OnCompleteListener { task ->
            if (!task.isSuccessful) {
                Log.d(tag, "Failed to fetch FCM registration token", task.exception)
                Toast.makeText(baseContext, "Failed to fetch FCM registration token", Toast.LENGTH_SHORT).show()
            ...
            if (token != null) {
                ...
                // Copy push token to the clipboard (only for test purposes)
